...
A operação mínima da Lívia Platform usa o Django Admin nativo. Esta fase não cria dashboard customizado, não ativa IA real e não ativa dispatch real.
...
Abra **Conversations > Conversations** para filtrar por tenant, `lead_state` e qualificação. O detalhe da conversa exibe mensagens em linha.
...
Abra **Leads > Lead drafts** para acompanhar nome, empresa, telefone, e-mail, status e área de serviço calculada.

A área de serviço é derivada do resumo da necessidade, sem criar campo novo no banco. Use os filtros por tenant, status e service area para triagem.

## Consultar handoffs

Abra **Conversations > Handoff requests** para ver pedidos de atendimento humano. Filtros disponíveis: status, motivo, prioridade e tenant.
...
Esta operação administrativa não ativa dispatch real para CRM ou notificações. Mantenha dry-run nas integrações enquanto o cutover operacional não estiver validado.
